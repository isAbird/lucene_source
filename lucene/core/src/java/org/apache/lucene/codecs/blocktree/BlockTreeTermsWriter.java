 ...
 * <p>
 * Writes terms dict and index, block-encoding (column stride) each term's metadata for each set of terms between two
 * index terms.
 * <p>
 *
 * If {@code minItemsInAutoPrefix} is not zero, then for {@link IndexOptions#DOCS} fields we detect prefixes that match
 * "enough" terms and insert auto-prefix terms into the index, which are used by {@link Terms#intersect} at search time
 * to speed up prefix and range queries. Besides {@link Terms#intersect}, these auto-prefix terms are invisible to all
 * other APIs (don't change terms stats, don't show up in normal {@link TermsEnum}s, etc.).
 * <p>
 *
 * Files:
 * <ul>
 ...
 *
 * <p>
 * The .tim file contains the list of terms in each field along with per-term statistics (such as docfreq) and per-term
 * metadata (typically pointers to the postings list for that term in the inverted index).
 * </p>
 *
 * <p>
 * The .tim is arranged in blocks: with blocks containing a variable number of entries (by default 25-48), where each
 * entry is either a term or a reference to a sub-block.
 * </p>
 *
 * <p>
 * NOTE: The term dictionary can plug into different postings implementations: the postings writer/reader are actually
 * responsible for encoding and decoding the Postings Metadata and Term Metadata sections.
 * </p>
 *
 * <ul>
 ...
 * <li>OuterNode --&gt; EntryCount, SuffixLength, Byte<sup>SuffixLength</sup>, StatsLength, &lt; TermStats
 * &gt;<sup>EntryCount</sup>, MetaLength, &lt;<i>TermMetadata</i>&gt;<sup>EntryCount</sup></li>
 * <li>InnerNode --&gt; EntryCount, SuffixLength[,Sub?], Byte<sup>SuffixLength</sup>, StatsLength, &lt; TermStats ?
 * &gt;<sup>EntryCount</sup>, MetaLength, &lt;<i>TermMetadata ? </i>&gt;<sup>EntryCount</sup></li>
 ...
 * <li>EntryCount,SuffixLength,StatsLength,DocFreq,MetaLength,NumFields, FieldNumber,RootCodeLength,DocCount,LongsSize
 * --&gt; {@link DataOutput#writeVInt VInt}</li>
 * <li>TotalTermFreq,NumTerms,SumTotalTermFreq,SumDocFreq --&gt; {@link DataOutput#writeVLong VLong}</li>
 ...
 * </p>
 * <ul>
 * <li>Header is a {@link CodecUtil#writeHeader CodecHeader} storing the version information for the BlockTree
 * implementation.</li>
 ...
 * <li>TotalTermFreq is the total number of occurrences of the term. This is encoded as the difference between the total
 * number of occurrences and the DocFreq.</li>
 ...
 * <li>SumDocFreq is the total number of postings, the number of term-document pairs across the entire field.</li>
 * <li>DocCount is the number of documents that have at least one posting for this field.</li>
 * <li>LongsSize records how many long values the postings writer/reader record per term (e.g., to hold freq/prox/doc
 * file offsets).
 * <li>MinTerm, MaxTerm are the lowest and highest term in this field.</li>
 * <li>PostingsHeader and TermMetadata are plugged into by the specific postings implementation: these contain arbitrary
 * per-file data (such as parameters or versioning information) and per-term data (such as pointers to inverted
 * files).</li>
 * <li>For inner nodes of the tree, every entry will steal one bit to mark whether it points to child nodes(sub-block).
 * If so, the corresponding TermStats and TermMetaData are omitted</li>
 * </ul>
 * <a name="Termindex"></a>
 * <h3>Term Index</h3>
 * <p>
 * The .tip file contains an index into the term dictionary, so that it can be accessed randomly. The index is also used
 * to determine when a given term cannot exist on disk (in the .tim file), saving a disk seek.
 * </p>
 * <ul>
 * <li>TermsIndex (.tip) --&gt; Header, FSTIndex<sup>NumFields</sup> &lt;IndexStartFP&gt;<sup>NumFields</sup>,
 * DirOffset, Footer</li>
 * <li>Header --&gt; {@link CodecUtil#writeHeader CodecHeader}</li>
 * <li>DirOffset --&gt; {@link DataOutput#writeLong Uint64}</li>
 * <li>IndexStartFP --&gt; {@link DataOutput#writeVLong VLong}</li> <!-- TODO: better describe FST output here -->
 ...
 * </p>
 * <ul>
 * <li>The .tip file contains a separate FST for each field. The FST maps a term prefix to the on-disk block that holds
 * all terms starting with that prefix. Each field's IndexStartFP points to its FST.</li>
 * <li>DirOffset is a pointer to the start of the IndexStartFPs for all fields</li>
 * <li>It's possible that an on-disk block would contain too many terms (more than the allowed maximum (default: 48)).
 * When this happens, the block is sub-divided into new blocks (called "floor blocks"), and then the output in the FST
 * for the block's prefix encodes the leading byte of each sub-block, and its file pointer.
 * </ul>
 *
...
public final class BlockTreeTermsWriter extends FieldsConsumer {
   ...
   * Suggested default value for the {@code
    *  minItemsInBlock} parameter to {@link #BlockTreeTermsWriter(SegmentWriteState,PostingsWriterBase,int,int)}.
   */
  public final static int DEFAULT_MIN_BLOCK_SIZE = 25;
  
  /**
   * Suggested default value for the {@code
    *  maxItemsInBlock} parameter to {@link #BlockTreeTermsWriter(SegmentWriteState,PostingsWriterBase,int,int)}.
   */
  ...
  private static class FieldMetaData {
    ...
    public FieldMetaData(FieldInfo fieldInfo, BytesRef rootCode, long numTerms, long indexStartFP,
        long sumTotalTermFreq, long sumDocFreq, int docCount, int longsSize, BytesRef minTerm, BytesRef maxTerm) {
      assert numTerms > 0;
  ...
  /**
   * Create a new writer. The number of items (terms or sub-blocks) per block will aim to be between minItemsPerBlock
   * and maxItemsPerBlock, though in some cases the blocks may be smaller than the min. For DOCS_ONLY fields, this terms
   * dictionary will insert automatically generated prefix terms for common prefixes, as long as each prefix matches at
   * least {@code minItemsInAutoPrefix} other terms or prefixes, and at most {@code maxItemsInAutoPrefix} other terms or
   * prefixes. Set {@code minItemsInAutoPrefix} to 0 to disable auto-prefix terms.
   */
  public BlockTreeTermsWriter(SegmentWriteState state, PostingsWriterBase postingsWriter, int minItemsInBlock,
      int maxItemsInBlock, int minItemsInAutoPrefix, int maxItemsInAutoPrefix) throws IOException {
    // 块太大小导致分裂、共同前缀太大导致分裂
    ...
    final String termsName = IndexFileNames.segmentFileName(state.segmentInfo.name, state.segmentSuffix,
    ...
    try {
      ...
      final String indexName = IndexFileNames.segmentFileName(state.segmentInfo.name, state.segmentSuffix,
          BlockTreeTermsReader.TERMS_INDEX_EXTENSION);
      indexOut = state.directory.createOutput(indexName, state.context);
      CodecUtil.writeIndexHeader(indexOut, BlockTreeTermsReader.TERMS_INDEX_CODEC_NAME,
          BlockTreeTermsReader.VERSION_CURRENT, state.segmentInfo.getId(), state.segmentSuffix);
  ...
  public static void validateSettings(int minItemsInBlock, int maxItemsInBlock) {
    ...
    if (minItemsInBlock > maxItemsInBlock) {
      throw new IllegalArgumentException("maxItemsInBlock must be >= minItemsInBlock; got maxItemsInBlock="
          + maxItemsInBlock + " minItemsInBlock=" + minItemsInBlock);
    }
    if (2 * (minItemsInBlock - 1) > maxItemsInBlock) {
      throw new IllegalArgumentException("maxItemsInBlock must be at least 2*(minItemsInBlock-1); got maxItemsInBlock="
          + maxItemsInBlock + " minItemsInBlock=" + minItemsInBlock);
    }
  }
    ...
    if (minItemsInAutoPrefix != 0) {
      if (minItemsInAutoPrefix < 2) {
        throw new IllegalArgumentException(
            "minItemsInAutoPrefix must be at least 2; got minItemsInAutoPrefix=" + minItemsInAutoPrefix);
      }
      if (minItemsInAutoPrefix > maxItemsInAutoPrefix) {
        throw new IllegalArgumentException(
            "maxItemsInAutoPrefix must be >= minItemsInAutoPrefix; got maxItemsInAutoPrefix=" + maxItemsInAutoPrefix
                + " minItemsInAutoPrefix=" + minItemsInAutoPrefix);
      }
      if (2 * (minItemsInAutoPrefix - 1) > maxItemsInAutoPrefix) {
        throw new IllegalArgumentException(
            "maxItemsInAutoPrefix must be at least 2*(minItemsInAutoPrefix-1); got maxItemsInAutoPrefix="
                + maxItemsInAutoPrefix + " minItemsInAutoPrefix=" + minItemsInAutoPrefix);
      }
  ...
  public void write(Fields fields) throws IOException {
    ...
    for (String field : fields) {
      ...
      if (minItemsInAutoPrefix != 0) {
        if (fieldInfo.getIndexOptions() != IndexOptions.DOCS) {
          throw new IllegalStateException(
              "ranges can only be indexed with IndexOptions.DOCS (field: " + fieldInfo.name + ")");
        }
      ...
      while (true) {
        ...
        if (prefixTerms != null) {
          while (prefixTermUpto < prefixTerms.size()
              && (term == null || prefixTerms.get(prefixTermUpto).compareTo(term) <= 0)) {
            PrefixTerm prefixTerm = prefixTerms.get(prefixTermUpto);
            // if (DEBUG) System.out.println("seg=" + segment + " field=" + fieldInfo.name + " NOW INSERT prefix=" +
            // prefixTerm);
  ...
  static long encodeOutput(long fp, boolean hasTerms, boolean isFloor) {
    assert fp < (1L << 62);
    return (fp << 2) | (hasTerms ? BlockTreeTermsReader.OUTPUT_FLAG_HAS_TERMS : 0)
  ...
  private static final class PendingBlock extends PendingEntry {
    ...
    public PendingBlock(BytesRef prefix, long fp, boolean hasTerms, boolean isFloor, int floorLeadByte,
    ...
    public void compileIndex(List<PendingBlock> blocks, RAMOutputStream scratchBytes, IntsRefBuilder scratchIntsRef)
        throws IOException {
      
      assert (isFloor && blocks.size() > 1) || (isFloor == false && blocks.size() == 1) : "isFloor=" + isFloor
          + " blocks=" + blocks;
      ...
      final Builder<BytesRef> indexBuilder = new Builder<>(FST.INPUT_TYPE.BYTE1, 0, 0, true, false, Integer.MAX_VALUE,
          outputs, false, PackedInts.COMPACT, true, 15);
      // if (DEBUG) {
      ...
      /*
       * Writer w = new OutputStreamWriter(new FileOutputStream("out.dot")); Util.toDot(index, w, false, false);
       * System.out.println("SAVED to out.dot"); w.close();
       */
    }
    ...
    // FST.
    private void append(Builder<BytesRef> builder, FST<BytesRef> subIndex, IntsRefBuilder scratchIntsRef)
        throws IOException {
  ...
  class TermsWriter {
    ...
    void writeBlocks(int prefixLength, int count) throws IOException {
      ...
      for (int i = start; i < end; i++) {
        ...
        if (suffixLeadLabel != lastSuffixLeadLabel) {
          ...
          if (itemsInBlock >= minItemsInBlock && end - nextBlockStart > maxItemsInBlock) {
            ...
            newBlocks.add(writeBlock(prefixLength, isFloor, nextFloorLeadLabel, nextBlockStart, i, hasTerms,
                hasPrefixTerms, hasSubBlocks));
      ...
      if (nextBlockStart < end) {
        ...
        boolean isFloor = itemsInBlock < count;
        newBlocks.add(writeBlock(prefixLength, isFloor, nextFloorLeadLabel, nextBlockStart, end, hasTerms,
            hasPrefixTerms, hasSubBlocks));
      }
     ...
     * is true, there were too many (more than maxItemsInBlock) entries sharing the same prefix, and so we broke it into
     * multiple floor blocks where we record the starting label of the suffix of each floor block.
     */
    private PendingBlock writeBlock(int prefixLength, boolean isFloor, int floorLeadLabel, int start, int end,
      ...
      // if (DEBUG2) System.out.println(" writeBlock field=" + fieldInfo.name + " prefix=" + brToString(prefix) + " fp="
      // + startFP + " isFloor=" + isFloor + " isLastInFloor=" + (end == pending.size()) + " floorLeadLabel=" +
      // floorLeadLabel + " start=" + start + " end=" + end + " hasTerms=" + hasTerms + " hasSubBlocks=" +
      // hasSubBlocks);
      ...
      /*
       * if (DEBUG) { System.out.println("  writeBlock " + (isFloor ? "(floor) " : "") + "seg=" + segment +
       * " pending.size()=" + pending.size() + " prefixLength=" + prefixLength + " indexPrefix=" + brToString(prefix) +
       * " entCount=" + (end-start+1) + " startFP=" + startFP + (isFloor ? (" floorLeadLabel=" +
      ...
      if (isLeafBlock) {
        ...
        for (int i = start; i < end; i++) {
          ...
          if (ent.isTerm) {
            ...
            assert StringHelper.startsWith(term.termBytes, prefix) : "term.term=" + term.termBytes + " prefix="
                + prefix;
            BlockTermState state = term.state;
            final int suffix = term.termBytes.length - prefixLength;
            // if (DEBUG2) {
            ...
            // suffixBytes.length = suffix;
            // System.out.println(" write sub-block suffix=" + brToString(suffixBytes) + " subFP=" + block.fp + "
            // subCode=" + (startFP-block.fp) + " floor=" + block.isFloor);
            // }
            
            assert floorLeadLabel == -1
                || (block.prefix.bytes[prefixLength] & 0xff) >= floorLeadLabel : "floorLeadLabel=" + floorLeadLabel
                    + " suffixLead=" + (block.prefix.bytes[prefixLength] & 0xff);
    ...
    public void write(BytesRef text, TermsEnum termsEnum, PrefixTerm prefixTerm) throws IOException {
      /*
       * if (DEBUG) { int[] tmp = new int[lastTerm.length]; System.arraycopy(prefixStarts, 0, tmp, 0, tmp.length);
       * System.out.println("BTTW: write term=" + brToString(text) + " prefixStarts=" + Arrays.toString(tmp) +
      ...
      if (state != null) {
        ...
        assert fieldInfo.getIndexOptions() == IndexOptions.DOCS
            || state.totalTermFreq >= state.docFreq : "postingsWriter=" + postingsWriter;
        pushTerm(text);
    ...
    private void pushTerm(BytesRef text) throws IOException {
      ...
      for (int i = lastTerm.length() - 1; i >= pos; i--) {
        ...
        if (prefixTopSize >= minItemsInBlock) {
          // if (DEBUG) System.out.println("pushTerm i=" + i + " prefixTopSize=" + prefixTopSize + " minItemsInBlock=" +
          // minItemsInBlock);
    ...
    public void finish() throws IOException {
      if (numTerms > 0) {
        ...
        assert pending.size() == 1 && !pending.get(0).isTerm : "pending.size()=" + pending.size() + " pending="
            + pending;
        ...
        /*
         * if (DEBUG) { final String dotFileName = segment + "_" + fieldInfo.name + ".dot"; Writer w = new
         * OutputStreamWriter(new FileOutputStream(dotFileName)); Util.toDot(root.index, w, false, false);
         * System.out.println("SAVED to " + dotFileName); w.close(); }
         */
        ...
        fields.add(new FieldMetaData(fieldInfo, ((PendingBlock) pending.get(0)).index.getEmptyOutput(), numTerms,
            indexStartFP, sumTotalTermFreq, sumDocFreq, docsSeen.cardinality(), longsSize, minTerm, maxTerm));
      } else {
